...
public class Atm extends Thread {
  ...
  public void login(Integer account) {
    ...
    try (Socket authConnection = new Socket("localhost",
        ...
        PrintWriter writer = new PrintWriter(authConnection.getOutputStream(),
            true);
        BufferedReader reply = new BufferedReader(new InputStreamReader(
            authConnection.getInputStream()));) {
  ...
  private void withdrawAmount(Integer amount) {
    try (Socket puConnection = new Socket("localhost", Globals.atmToNetwork)) {
      PrintWriter writer = new PrintWriter(puConnection.getOutputStream(), true);
